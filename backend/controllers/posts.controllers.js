...
postsCtrl.getPostsDescarga = (request, response) => {
    //console.log("entra aca")
    const {id_descarga} = request.params
    pool.query(`SELECT * FROM posts where posts.id_descarga='${id_descarga}'`, (error, result) => {
        if (error) throw error;
        response.send(result);
    });
}
